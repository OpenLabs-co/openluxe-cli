...
export const RESOURCES = {
    ...
    storylines: {
        summary: 'storylines (v1) — swipeable story decks (/stories builder)',
        commands: {
            list: { method: 'GET', path: '/storylines', summary: 'GET /storylines — your decks; filters: status, updated_since  [scope: storylines:read]' },
            get: { method: 'GET', path: '/storylines/:uuid', summary: 'GET /storylines/{uuid} — deck + ordered step frames  [scope: storylines:read]' },
            create: { method: 'POST', path: '/storylines', summary: "Create a draft deck (-d '{\"title\":\"…\",\"steps\":[{\"frame\":{\"type\":\"statement\",\"headline\":\"…\"}}]}')  [scope: storylines:write]" },
            update: { method: 'PATCH', path: '/storylines/:uuid', summary: "Update shell/publish/move to a group (-d '{\"status\":\"published\"}' goes live + QR; settings carries white-label branding{logo_url,label,link_url} + persistent cta{enabled,label,href,position})  [scope: storylines:write]" },
            replace: { method: 'PUT', path: '/storylines/:uuid', summary: 'PUT alias of update  [scope: storylines:write]' },
            'set-steps': { method: 'PUT', path: '/storylines/:uuid/steps', summary: "Replace ALL step frames (-d '{\"steps\":[{\"frame\":{…}}]}') — sanitized like the in-app editor  [scope: storylines:write]" },
            duplicate: { method: 'POST', path: '/storylines/:uuid/duplicate', summary: 'Deep-copy a deck (steps by value; not published, no QR/views copied)  [scope: storylines:write]' },
            qr: { method: 'POST', path: '/storylines/:uuid/qr', summary: '(Re)generate the QR PNG → returns its CDN url  [scope: storylines:write]' },
            analytics: { method: 'GET', path: '/storylines/:uuid/analytics', summary: 'Viewer analytics — totals, views-over-time, step funnel, viewers (?days=30&per_page=25)  [scope: storylines:read]' },
            responses: { method: 'GET', path: '/storylines/:uuid/responses', summary: 'Form submissions captured by the deck (?per_page=25)  [scope: storylines:read]' },
            'frame-types': { method: 'GET', path: '/storylines/frame-types', summary: 'Frame schema reference — every frame type with fields/enums/limits/examples (map, gallery mode+fill, capture, signup, video, …); read before authoring frames  [scope: storylines:read]' },
            schedules: { method: 'GET', path: '/storylines/:uuid/schedules', summary: "List a deck's scheduled recurring surveys  [scope: storylines:read]" },
            'schedule-responses': { method: 'GET', path: '/storylines/:uuid/schedules/:schedule/responses', summary: 'Responses-over-time for a schedule (sent/opened/responded per occurrence)  [scope: storylines:read]' },
            'create-schedule': { method: 'POST', path: '/storylines/:uuid/schedules', summary: "Schedule a recurring survey send (-d '{\"cadence\":\"monthly\",\"contact_list_id\":123}'); deck must be published — this EMAILS the audience  [scope: storylines:schedules:write]" },
            'update-schedule': { method: 'PATCH', path: '/storylines/:uuid/schedules/:schedule', summary: "Rename or set status (-d '{\"status\":\"paused|active|ended\"}')  [scope: storylines:schedules:write]" },
            'replace-schedule': { method: 'PUT', path: '/storylines/:uuid/schedules/:schedule', summary: 'PUT alias of update-schedule  [scope: storylines:schedules:write]' },
            'delete-schedule': { method: 'DELETE', path: '/storylines/:uuid/schedules/:schedule', summary: 'Delete a schedule (stops future sends; past responses kept)  [scope: storylines:schedules:write]' },
            delete: { method: 'DELETE', path: '/storylines/:uuid', summary: 'DELETE /storylines/{uuid}  [scope: storylines:write]' },
            groups: { method: 'GET', path: '/storylines/groups', summary: 'List your deck folders  [scope: storylines:read]' },
            'create-group': { method: 'POST', path: '/storylines/groups', summary: "Create a folder (-d '{\"name\":\"…\",\"color\":\"#6366f1\"}')  [scope: storylines:write]" },
            'update-group': { method: 'PATCH', path: '/storylines/groups/:group', summary: 'Rename/recolor a folder  [scope: storylines:write]' },
            'replace-group': { method: 'PUT', path: '/storylines/groups/:group', summary: 'PUT alias of update-group  [scope: storylines:write]' },
            'delete-group': { method: 'DELETE', path: '/storylines/groups/:group', summary: 'Delete a folder (its decks fall back to Unfiled)  [scope: storylines:write]' },
            palettes: { method: 'GET', path: '/storylines/palettes', summary: 'List your custom color palettes (use key custom-{id} as a deck scheme)  [scope: storylines:read]' },
            'create-palette': { method: 'POST', path: '/storylines/palettes', summary: "Create a palette (-d '{\"name\":\"…\",\"tokens\":{\"bg\":\"#…\",\"bgDeep\":\"#…\",\"bgSoft\":\"#…\",\"ink\":\"#…\",\"inkSoft\":\"#…\",\"accent\":\"#…\"}}')  [scope: storylines:write]" },
            'delete-palette': { method: 'DELETE', path: '/storylines/palettes/:palette', summary: 'Delete a custom palette  [scope: storylines:write]' },
        },
    },
    'business-dna': {
...
export const WEB = {
    ...
    storylines: { hub: '/stories', item: (r) => (r.id ? `/stories/${r.id}/edit` : null), open: '/stories', openSummary: 'Open the Storyline Builder in your browser (deck records carry public_url + web_url links)' },
